<div class="container-fluid">
    <div class="p-4 mb-4">
        ...
        <div class="row" wire:ignore>
            ...
            <div class="row p-2 mb-3">
                ...
                @can('status_government_broker', auth()->user())
                    <div class="col-md-3">
        ...
        <div class=" table-responsive-md text-center">
            <table class="table table-bordered text-center" style="margin-bottom: 0rem;">
                ...
                <tbody wire:loading.remove>
                    @forelse ($government_brokers as $government_broker)
                        <tr>
                            <td>{{ $government_broker->id }}</td>
                            <td>
                                <div class="lightbox">
                                    <img src="{{ $government_broker->photo_table }}"
                                        data-mdb-img="{{ $government_broker->photo_table }}" width="30"
                                        height="30">
                                </div>
                            </td>
                            <td>{{ $government_broker->name }}</td>

                            @can('status_government_broker', auth()->user())
                                <td class="switch">
                                    <label>
                                        نشط
                                        <input wire:click="changeStatus({{ $government_broker->id }})" type="checkbox"
                                            {{ $government_broker->status == 'active' ? 'checked' : '' }}>
                            ...
                            @canany(['edit_government_broker', 'delete_government_broker'], auth()->user())
                                <td>
                                    <div class="d-flex justify-content-center">
                                        @can('delete_government_broker', auth()->user())
                                            <a type="button" class="text-danger fa-lg me-2 ms-2"
                                                wire:click='delete({{ $government_broker->id }})' title="Delete">
                                                <i class="fas fa-trash-can"></i>
                                            </a>
                                        @endcan

                                        @can('edit_government_broker', auth()->user())
                                            <a type="button" class="text-dark fa-lg me-2 ms-2 set-button-update"
                                                data-mdb-toggle="modal" data-mdb-target="#create-new-government-broker-modal"
                                                href="#create-new-government-broker-modal" title="EditGovernmentBroker"
                                                wire:click="edit({{ $government_broker->id }})">
                        ...
                        <tr>
                            <td colspan="5" class="fw-bold fs-6">لا يوجد نتائج !!</td>
                        </tr>
                    @endforelse
        ...
        <div class="d-flex justify-content-between mt-4">
            ...
            <nav aria-label="...">
                <ul class="pagination pagination-circle">
                    {{ $government_brokers->withQueryString()->onEachSide(0)->links() }}
                </ul>
            </nav>
    ...
    @canany(['create_government_broker', 'edit_government_broker'], auth()->user())
        <div class="modal top fade" id="create-new-government-broker-modal" tabindex="-1" data-mdb-backdrop="static"
            ...
            <div class="modal-dialog cascading-modal" style="margin-top: 4%">
                <div class="modal-content">
                    <div class="modal-c-tabs">
                        ...
                        <!-- Tabs navs -->
                        <ul class="nav md-tabs nav-tabs" id="create-new-government-broker" role="tablist"
                            style="background-color: #303030;">
                            <li class="nav-item" role="presentation">
                                <a data-mdb-tab-init class="nav-link active" id="create-new-government-broker-tab-1"
                                    href="#create-new-government-broker-tabs-1" role="tab"
                                    aria-controls="create-new-government-broker-tabs-1" aria-selected="true">
                            ...
                            <div class="tab-pane fade show active" id="create-new-government-broker-tabs-1" role="tabpanel"
                                ...
                                <div class="modal-body">
                                    <div class="row mb-3">
                                        <div class="col-md-12">
                                            ...
                                            <label class="form-label" for="forName"><strong>اسم الجهة
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="name" wire:model.defer="name" maxlength="15"
                                                    class="form-control government-broker-name"
                                                    placeholder="ادخل اسم الجهة الحكومية" />

                                            </div>
                                            <div
                                                class="form-helper text-danger name-create-new-government-broker-validation reset-validation">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-12">
                                            <label class="form-label"
                                                for="forGovernmentBrokerStatus"><strong>الحالة</strong></label>
                                            <select class="select" id="add-government-broker-status" wire:model="status">
                                                <option value="active">نشط</option>
                                                <option value="inactive">غير نشط</option>
                                            </select>
                                            <div
                                                class="form-helper text-danger status-create-new-government-broker-validation reset-validation">
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="button" class="btn text-white blue-color create-button"
                                        wire:click='addGovernmentBroker()'>حفظ</button>

                                    <button type="button" class="btn text-white blue-color update-button"
                                        wire:click='updateGovernmentBroker()'>تحديث</button>
    ...
    <script>
        $(document).ready(function() {
            ...
            Livewire.on("process-has-been-done", function() {
                ...
                $("#create-new-government-broker-modal").modal('hide');
                $(".create-button").show();
                $(".update-button").hide();
            });

            Livewire.on("create-new-government-broker-errors", function(errors) {
                $(".reset-validation").text("");
                for (let key in errors[0]) {
                    if (errors[0].hasOwnProperty(key)) {
                        $("." + key + "-create-new-government-broker-validation").text(errors[0][key]);
                    }
                }
            });

            Livewire.on("set-government-broker-status", function(data) {
                const singleSelect = document.querySelector("#add-government-broker-status");
